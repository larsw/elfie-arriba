...
namespace Arriba.Server.Authentication
{
    ...
    internal class RuntimeCache : IDisposable
    {
        ...
        private readonly TimeSpan _maximumTimeToLive = TimeSpan.FromDays(180);
        ...
        /// <summary>
        ///     Gets an existing cache item for the specified key, if it does not exist a cache item is added by running the
        ...
        public T GetOrAdd<T>(string key, Func<T> production, TimeSpan? timeToLive = null)
        {
            ...
            {
                value = production();
                _cache.Add(key, value, CreatePolicyForValue(value, timeToLive));
            }
