...
namespace Arriba.Communication
{
    ...
    public class ApplicationServer : IRequestHandler, IDisposable
    {
        ...
        /// </summary>
        private static readonly IResponse s_notFoundResponse =
            new Response(ResponseStatus.NotFound, "Resource not found");
        ...
        private readonly EventPublisherSource _eventSource = EventPublisher.CreateEventSource(new MonitorEventEntry
            {Source = "Application", OpCode = MonitorEventOpCode.Mark});
        ...
        public IContentReaderWriterService ReaderWriter => _readerWriter;
        ...
        public async Task<IResponse> HandleAsync(IRequest request, bool passThrough)
        {
            ...
            using (ctx.Monitor(MonitorEventLevel.Verbose, "Global.EndToEnd"))
            {
                ...
                if (request.Method == RequestVerb.Head) request = new ModifiedVerbRequest(request, RequestVerb.Get);
                ...
                foreach (var application in _applications)
                {
                    ...
                    {
                        if (originalVerb == RequestVerb.Head) response = new NullBodyResponse(response);
        ...
        public void RegisterContentReader(IContentReader reader)
        {
            ...
            Trace.TraceInformation("Added content reader \"{0}\" for content types [{1}]", reader.GetType().Name,
                string.Join(", ", reader.ContentTypes));
        }
        ...
        public void RegisterContentWriter(IContentWriter writer)
        {
            ...
            Trace.TraceInformation("Added content writer \"{0}\" for content type [{1}]", writer.GetType().Name,
                writer.ContentType);
        }
        ...
        public async Task StartAsync()
        {
            ...
            if (_channels.Count == 0)
                throw new InvalidOperationException(
                    "No application channels have been registered to the application server");
            ...
            _channelTasks = _channels.Select(s => s.StartAsync(this, _readerWriter, _cancellationSource.Token))
                .ToArray();

            Trace.TraceInformation("Application server running with {0} applications and {1} channels...",
                _applications.Count, _channels.Count);
        ...
        private async Task<IResponse> HandleApplicationRequestAsync(IRequestContext request, IApplication application)
        {
            ...
            {
                var errorMessage = string.Format("Application \"{0}\" return a null response result");
            ...
            {
                Trace.TraceError("Unexpected {0} for request [1]", handleException.GetType().Name,
                    GetRequestIdentityString(request));
                ...
                {
                    Request = request.Request.Resource, RequestParms = request.Request.ResourceParameters.ValuePairs,
        ...
        {
            return string.Format("Resource: {0}, Method: {1}, User: {2}", ctx.Request.Resource, ctx.Request.Method,
                ctx.Request.User.Identity.Name);
        }

        private void GuardIsRunning()
        {
            if (!_isRunning) throw new InvalidOperationException("Application server is running");
        }

        private void GuardIsNotRunning()
        {
            if (_isRunning) throw new InvalidOperationException("Application server is not running");
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (_cancellationSource != null) _cancellationSource.Dispose();
